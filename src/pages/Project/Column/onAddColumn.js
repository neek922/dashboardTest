...
const onAddColumn = (e) =>  {
	...
	Store().dispatch({
		...
		payload: () => {
			if(value){
				currentState.data.push({id: currentState.data.length+1, name: value });
			
				currentState.data = [ ...currentState.data ];
				return { ...currentState};
			} else {
				currentState.data.push({id: currentState.data.length+1, name: `Column${currentState.data.length+1}`});
